...
<html lang="eng">
<head>
    <title>Leave Request Overview</title>
...
<style>
    .custom-container {
        width:max-content; /* Adjust the value to your desired width */
                ...
                <div class="card my-4">
                    <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                        <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                            <h6 class="text-white text-capitalize ps-3">leave Request Overview</h6>
                        </div>
                    </div>
                        ...
                        <div class="table-responsive p-0">
                            <table class="table align-items-center mb-0">
                                ...
                                <tr>
                                    ...
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ">Apply Date</th>
                                    <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Status</th>
                                    <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Leave From</th>
                                    <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Leave To</th>
                                    <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Leave Type</th>
                                ...
                                $sql =<<<Eof
                                    SELECT * FROM leavemanagement
                                     INNER JOIN employeeslist ON leavemanagement.employeesid = employeeslist.employeesid
                                Eof;
                                $ret = pg_query($conn, $sql);
                                if(!$ret) {
                                    echo pg_last_error($conn);
                                ...
                                while ($let=pg_fetch_assoc($ret)){
                                    ...
                                    $email=$let['email'];
                                    $fullname=$let['fullname'];
                                    $applydate=date("Y M D d", strtotime($let['applydate']));
                                    $leavefrom=date("Y M D d", strtotime($let['leavefrom']));
                                    $leaveto=date("Y M D d", strtotime($let['leaveto']));
                                    $status=$let['status'];
                                    $leavetype=$let['leavetype'];
                                    $remark=$let['remark'];
                                  ...
                                  echo "
                                      <td class='align-middle text-center'>
                                        <span class='text-secondary text-xs font-weight-bold'>".$applydate."</span>
                                    </td>
                                      <td class='align-middle text-center'>
                                     ...
                                     <td class='align-middle text-center'>
                                        <span class='text-secondary text-xs font-weight-bold'> ".$leavefrom."</span>
                                    </td>
                                        <td class='align-middle text-center'>
                                        <span class='text-secondary text-xs font-weight-bold'>". $leaveto."</span>
                                    </td>
                                       </td>
                                        <td class='align-middle text-center'>
                                        <span class='text-secondary text-xs font-weight-bold'>". $leavetype."</span>
                                    </td>
                                          </td>
                                        <td class='align-middle text-center'>
                                        <span class='text-secondary text-xs font-weight-bold'>". $remark."</span>
                                    </td>
                                    ";


                                    echo "<td class='align-middle'>
                                        <form method='post' action='approve-decline.php'>
                                            <button class='btn btn-lg bg-gradient-primary btn-sm w-90 mt-2 mb-0' name='approved' value='$id'><i class='material-icons text-lg position-relative'>check</i></button>
                                        </form>
                                    </td>";

                                    echo "<td class='align-middle'>
                                             <form method='post' action='approve-decline.php'>
                                        <button class='btn btn-lg bg-gradient-primary btn-sm w-90 mt-2 mb-0' name='declined' value='$id'><i class='material-icons text-lg position-relative'>delete</i></button>
